<script lang="ts">
  ...
  async function quickComplete(habitId: number, e: MouseEvent) {
    e.stopPropagation();
...
{:else if !overview || overview.totalHabits === 0}
  <!-- No habits - compact placeholder -->
  <button
...
<!-- ═══════════════════════════════════════════════════════════════════ -->
<!-- MICRO: Single line - icon + streak + today fraction               -->
<!-- ═══════════════════════════════════════════════════════════════════ -->
...
<!-- ═══════════════════════════════════════════════════════════════════ -->
<!-- MINI: Ring + streak + today count, no heatmap, no quick-complete  -->
<!-- ═══════════════════════════════════════════════════════════════════ -->
...
<!-- ═══════════════════════════════════════════════════════════════════ -->
<!-- FULL: Ring + streak + heatmap + quick-complete buttons            -->
<!-- ═══════════════════════════════════════════════════════════════════ -->
{:else}
  <button 
    class="w-full rounded-lg border bg-card p-3 shadow-sm hover:shadow-md transition-all cursor-pointer text-left group"
    onclick={navigateToHabits}
    type="button"
  >
    <!-- Top row: Ring + Streak + Today count -->
    ...
    <!-- Mini Heatmap Grid (5 weeks × 7 days) -->
    ...
    {#if pendingToday.length > 0}
      <div class="space-y-1">
        {#each pendingToday.slice(0, 2) as habit (habit.id)}
          <button
            class="flex items-center justify-between w-full px-2 py-1 rounded-md text-[11px] bg-muted/50 hover:bg-primary/10 transition-colors group/item"
